...
build_kingdom_dataset <- function(edges, triplets, taxa, spieceasi_edges, kingdom_name, config) {
  ...
  # Final-community abundances are the supervised prediction targets, not input
  # features.
  final_edges <- edges_k[
...
# Combined training objective. Presence uses binary cross-entropy with a
# positive-class weight for sparse taxa; abundance loss is calculated only where
# the observed final community contains the taxon.
...
# Compute taxon-level classification metrics, abundance error for observed
# present taxa, and a community-level Bray-Curtis distance between predicted and
# observed final communities.
