...
# Train a presence/absence taxon-node graph neural network for microbial coalescence.
#
# The supervised unit is:
#   resident sample + pooled donor source -> observed final community
#
# Observed abundances are reinterpreted as binary presence/absence before model
# features and targets are constructed. SpiecEasi taxon-taxon edges are used as a signed, weighted association
...
#   Rscript workflows/code/train_coalescence_gnn_presence_absence_CMKR.R
#
# Example local smoke run:
#   Rscript workflows/code/train_coalescence_gnn_presence_absence_CMKR.R --kingdoms=Bacteria --max_taxa_per_kingdom=500 --epochs=5
#
# Example larger HPC run:
#   Rscript workflows/code/train_coalescence_gnn_presence_absence_CMKR.R --max_taxa_per_kingdom=Inf --epochs=200 --batch_size=16

# Check for packages that are not part of base R before doing any expensive
# file reads or model setup.
...
# Convert command-line arguments of the form --name=value into a named list.
# This lets the same script run locally, in a container, or on an HPC scheduler
...
# Return a user-supplied command-line value when present; otherwise keep the
# default value provided by the script or environment variable.
...
# Interpret common text values as true/false flags.
...
# Split comma-separated arguments such as "Bacteria,Fungi" into a character
# vector used for filtering.
parse_csv <- function(x) {
  values <- trimws(strsplit(as.character(x), ",", fixed = TRUE)[[1]])
  values[nzchar(values)]
}

# Allow numeric arguments to use Inf/all/none when the user does not want to cap
# the number of taxa.
...
# Timestamped progress messages are useful for long HPC runs and for diagnosing
# where a job stopped.
...
# Central configuration block. Each setting can be overridden from the command
# line or from an environment variable, which keeps the workflow reproducible
# while still being easy to scale up on HPC.
config <- list(
  ...
  output_dir = arg_value(
    cli_args,
    "output_dir",
    file.path(repo_root, "workflows", "output", "gnn_model_presence_absence")
  ),
  ...
  abundance_loss_weight = as.numeric(arg_value(
    ...
    Sys.getenv("GNN_ABUNDANCE_LOSS_WEIGHT", "0.0")
  )),
...
# Expected graph-input files generated by build_graph_inputs.R. These are read
# as model inputs; all model outputs are written to config$output_dir.
...
# Fix random seeds so repeated runs with the same settings are easier to compare.
set.seed(config$random_seed)
torch_manual_seed(config$random_seed)

# Use CUDA only when the user requested it and the installed torch runtime can
# see a GPU. Otherwise the workflow falls back to CPU.
...
# Load the supervised prediction units, taxon metadata, observed sample-taxon
# abundance edges, and inferred SpiecEasi taxon-taxon association edges.
...
sample_taxon_edges[, abundance := as.numeric(abundance > 0)]
...
# Keep only the kingdoms requested for this run before building dense arrays.
...
# Node features used for each taxon in each prediction unit. These describe only
# pre-coalescence information: resident state, donor-source state, and simple
# transformations of those abundances.
feature_names <- c(
  "resident_abundance",
  "donor_abundance",
  "resident_present",
...
# Choose the taxon universe for one kingdom. By default this keeps the most
# abundant pre-coalescence taxa to make local runs feasible; HPC runs can set
# max_taxa_per_kingdom=Inf.
...
# Transform SpiecEasi weights before adjacency normalization. The square-root
# default compresses very strong edges without discarding edge ordering.
...
# Convert each adjacency row into a weighted average over a taxon's neighbors.
# Rows with no neighbors remain all zero.
...
# Build signed taxon-taxon graph scaffolds for one kingdom. Positive and
# negative SpiecEasi associations become separate adjacency matrices so the
# neural network can learn different effects for each edge sign.
...
# Create train/validation/test splits at the microcosm level by default. An
# optional donor_id split can test whether the model generalizes across donor
# sources, but it requires enough donor groups to be meaningful.
...
# Fill one feature slice of the 3D input array:
#   example x taxon x feature.
# Sparse edge-list values that are absent from the CSV remain zero in the dense
# model tensor.
...
# Assemble all model-ready tensors for one kingdom. Each row of
# coalescence_triplets.csv becomes one graph-like training example with the same
# taxon universe and the same SpiecEasi scaffold.
build_kingdom_dataset <- function(edges, triplets, taxa, spieceasi_edges, kingdom_name, config) {
  ...
  # Dense arrays are easier for torch to batch. Missing taxon abundances are
  # true zeros because combined_sample_taxon_edges.csv only stores nonzero
  # abundance edges.
  ...
  # Resident features are specific to the resident sample in each triplet.
  ...
  # Donor features are donor-source-level means because this workshop dataset
  # has pooled donor-source observations rather than unique donor samples per
  # microcosm.
  donor_edges <- edges_k[
  ...
  # Final-community presence/absence values are the supervised prediction
  # targets, not input features.
  final_edges <- edges_k[
  ...
  # Derived features give the model both raw abundance scale and simple
  # presence/log-transformed contrasts.
  ...
  # Splits are assigned after graph construction so train/validation/test all
  # use the same taxon ordering and graph scaffold.
...
# Standardize node features using only training examples, then apply the same
# mean and standard deviation to validation and test examples.
...
# One signed message-passing block. Each taxon combines its own current
# embedding with weighted messages from positively associated neighbors and
# negatively associated neighbors.
signed_graph_block <- nn_module(
  ...
  forward = function(h, adj_pos, adj_neg) {
    ...
    # The adjacency matrices are shared across examples in a kingdom, then
    # expanded across the current mini-batch for batched matrix multiplication.
...
# Multi-task taxon-node GNN. It produces one prediction per taxon: final
# presence logits for classification and final log abundance for regression.
coalescence_gnn <- nn_module(
  ...
  forward = function(x, adj_pos, adj_neg) {
    ...
    # Learned taxon embeddings let the model capture taxon-specific tendencies
    # even when two taxa have similar donor/resident abundance features.
    ...
    # Message passing uses the signed SpiecEasi scaffold to let neighboring taxa
    # influence each other's predicted coalescence outcomes.
...
# Combined training objective. In this presence/absence variant, presence uses
# binary cross-entropy with a positive-class weight for sparse taxa and the
# abundance loss is disabled by default.
...
# Keep an independent copy of the best validation model parameters. Without
# cloning, later optimizer steps could mutate the saved tensors.
clone_state_dict <- function(state_dict) {
  lapply(state_dict, function(tensor) tensor$clone())
}

# Run model inference for an arbitrary set of example indices and return CPU
# arrays that can be evaluated or written to CSV.
...
# Compute taxon-level classification metrics and community-level similarity over
# the set of taxa predicted/observed as present. Jaccard similarity is the main
# presence-only replacement for Bray-Curtis dissimilarity here.
compute_metrics <- function(prediction, dataset, indices, split_name, config) {
  ...
  jaccard_similarity <- vapply(
    seq_len(nrow(observed_presence_bool)),
    function(i) {
      intersection <- sum(predicted_presence[i, ] & observed_presence_bool[i, ], na.rm = TRUE)
      union <- sum(predicted_presence[i, ] | observed_presence_bool[i, ], na.rm = TRUE)
      if (union == 0) {
        return(NA_real_)
      }
      intersection / union
  ...
  sorensen_similarity <- vapply(
    seq_len(nrow(observed_presence_bool)),
    function(i) {
      intersection <- sum(predicted_presence[i, ] & observed_presence_bool[i, ], na.rm = TRUE)
      total_present <- sum(predicted_presence[i, ], na.rm = TRUE) + sum(observed_presence_bool[i, ], na.rm = TRUE)
      if (total_present == 0) {
        return(NA_real_)
      }
      2 * intersection / total_present
    },
    numeric(1)
  )
  ...
  data.table(
    ...
    mean_jaccard_similarity = mean(jaccard_similarity, na.rm = TRUE),
    median_jaccard_similarity = stats::median(jaccard_similarity, na.rm = TRUE),
    mean_sorensen_similarity = mean(sorensen_similarity, na.rm = TRUE),
    median_sorensen_similarity = stats::median(sorensen_similarity, na.rm = TRUE)
  )
}

# Write long-format predictions so collaborators can inspect taxon-level
# outcomes, join predictions back to taxonomy, or make plots outside this script.
write_predictions <- function(prediction, dataset, split_labels, output_path, config) {
  n_examples <- nrow(dataset$triplets)
  n_taxa <- nrow(dataset$taxa)
  predicted_presence <- prediction$presence_probability >= config$prediction_threshold
  predicted_presence_binary <- matrix(
    as.numeric(predicted_presence),
    nrow = nrow(predicted_presence),
    ncol = ncol(predicted_presence)
  )
  ...
  output <- data.table(
    ...
    predicted_final_presence_binary = as.vector(t(predicted_presence_binary))
  )

  fwrite(output, output_path)
}

# Full train/evaluate/write cycle for one kingdom. Bacteria and fungi are
# trained separately so their taxon universes and SpiecEasi scaffolds do not get
# mixed.
train_one_kingdom <- function(dataset, config, device) {
  ...
  # Final presence is sparse, so the positive class is upweighted to avoid a
  # model that learns to predict absence everywhere.
  ...
  # Mini-batch training loop. Validation metrics are checked each epoch and the
  # best validation state is restored before final reporting.
  for (epoch in seq_len(config$epochs)) {
    ...
    validation_loss_proxy <- (1 - validation_metrics$presence_f1) +
      (1 - validation_metrics$mean_jaccard_similarity)
    ...
    history[[epoch]] <- data.table(
      ...
      validation_mean_jaccard_similarity = validation_metrics$mean_jaccard_similarity,
      validation_mean_sorensen_similarity = validation_metrics$mean_sorensen_similarity,
    ...
    if (epoch == 1 || epoch %% 5 == 0 || epoch == config$epochs) {
      log_message(
        ...
        " val_jaccard=",
        sprintf("%.4f", validation_metrics$mean_jaccard_similarity)
      )
    }
  }
  ...
  # After training, predict every split with the best validation model so the
  # output CSVs are internally consistent.
  ...
  # These files document exactly which SpiecEasi edges were retained after
  # kingdom/taxon/community-type filtering.
...
# Save the run settings before training so output folders document their own
# configuration.
...
# Train one model per requested kingdom and collect comparable metrics.
all_metrics <- list()
