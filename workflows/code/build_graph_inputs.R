...
write_readme <- function(output_dir, discovered_files, sample_taxon_edges, taxon_nodes, spieceasi_edges, summary_tbl, workflow_notes) {
  ...
  readme_lines <- c(
    ...
    "The configured default method is `glasso`.",
    ...
    "For each successful stratum, the selected precision matrix is converted to partial correlations with `P = -D^{-1/2} %*% Theta %*% D^{-1/2}`, where `D` is the diagonal of the precision matrix.",
    "Output edge weights are `abs(partial_correlation)` and `sign` records the direction of the partial correlation.",
    "",
    "Workflow parameters:",
    ...
    paste0("- `", taxon_taxon_edges_filename, "`: undirected SpiecEasi taxon-taxon edges with partial-correlation-derived weights."),
